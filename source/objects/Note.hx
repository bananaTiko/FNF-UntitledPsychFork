...
class Note extends FlxSprite
{
  ...
  public var z:Float = 0;
	...
	**/
	...
	public var hitsound:String = "hitsounds/hitsound-" + ClientPrefs.data.hitsoundType;

	public var opphitsoundDisabled:Bool = false;
	public var opphitsoundChartEditor:Bool = true;
	/**
	 * Forces the hitsound to be played even if the opponent's hitsound volume is set to 0
	**/
	public var opphitsoundForce:Bool = false;
	public var opphitsoundVolume(get, default):Float = 1.0;
	function get_opphitsoundVolume():Float {
		if(ClientPrefs.data.opphitsoundVolume > 0)
			return ClientPrefs.data.opphitsoundVolume;
		return opphitsoundForce ? opphitsoundVolume : 0.0;
	}
	public var opphitsound:String = "hitsounds/hitsound-" + ClientPrefs.data.oppHitsoundType;
		...
		if(noteData > -1 && noteType != value) {
			switch(value) {
				case 'Hurt Note':
					...
					opphitsound = 'cancelMenu';
					opphitsoundChartEditor = false;
				case 'Alt Animation':
					animSuffix = '-alt';
				case 'No Animation':
					noAnimation = true;
			...
			if (opphitsound != 'hitsound' && opphitsoundVolume > 0) Paths.sound(opphitsound); //precache new sound for being idiot-proof
			noteType = value;
		}
		return value;
	}
