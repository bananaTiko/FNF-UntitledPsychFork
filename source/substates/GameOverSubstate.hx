	...
	override function create()
	{
		...
		if(characterName == 'pico-dead')
		{
			...
			if(PlayState.instance.gf != null) {
				var gfChar:String = PlayState.instance.gf.curCharacter;
				if(gfChar == 'nene' || gfChar == 'nene-dark') {
				...
				else if(gfChar == 'nene-christmas') {
					var neneXmas:FlxSprite = new FlxSprite(boyfriend.x - 450, boyfriend.y - 250);
					neneXmas.frames = Paths.getSparrowAtlas('NeneKnifeToss');
					neneXmas.animation.addByPrefix('anim', 'knife toss xmas', 24, false);
					neneXmas.antialiasing = ClientPrefs.data.antialiasing;
					neneXmas.animation.finishCallback = function(_):Void {
						remove(neneXmas);
						neneXmas.destroy();
					};
					insert(0, neneXmas);
					neneXmas.animation.play('anim', true);
				}
			}
		super.create();
	}
