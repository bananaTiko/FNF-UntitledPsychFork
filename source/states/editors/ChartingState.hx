...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	[
		...
		['Windows Notification', "Value 1: Notification title\n - Defaults to \"Untitled Psych Fork\" if empty.\n\nValue 2: Notification message / info\n - Defaults to \"forgot to change values lmao\" if empty."],
	...
	override function create()
	{
		...
		mainBox = new PsychUIBox(mainBoxPosition.x, mainBoxPosition.y, 300, 300, ['Charting', 'Data', 'Events', 'Note', 'Section', 'Song']);
...
/*
			...
			*/
			if(FlxG.keys.justPressed.A != FlxG.keys.justPressed.D && !holdingAlt)
			{
					...
					{
						resetBfBuddy(); // makes lil bf stand up straight FUCGGGGGGGGGGGHKKKKKKED!!!!!!!!
						resetOppBuddy(); // makes lil opp stand up straight FUCGGGGGGGGGGGHKKKKKKED!!!!!!!!
						if(FlxG.sound.music.playing)
							setSongPlaying(false);
	...
	function hitNote(note:MetaNote) {
		...
		note.alpha = (note.strumTime >= Conductor.songPosition) ? 1 : 0.6;
			if(canPlayHitSound)
			{
				if(hitSoundPlayer && note.mustPress)
				{
					FlxG.sound.play(Paths.sound("hitsounds/hitsound-" + ClientPrefs.data.hitsoundType), hitsoundPlayerStepper.value).pan = (note.noteData < 4? -0.3 : 0.3) * ClientPrefs.data.hitsoundVolume;
					hitSoundPlayer = false;
				}
				else if(hitSoundOpp && !note.mustPress)
				{
					FlxG.sound.play(Paths.sound("hitsounds/hitsound-" + ClientPrefs.data.oppHitsoundType), hitsoundOpponentStepper.value).pan = (note.noteData < 4? -0.3 : 0.3) * ClientPrefs.data.opphitsoundVolume;
					hitSoundOpp = false;
				}
			}
			data = note.noteData;
		...
		public var lilCharDanceEveryNumBeats:Int = 2;

		function callBeatHit(curBeat) {
			if (curBeat % lilCharDanceEveryNumBeats == 0) {
				if (lilBf != null && lilBf.animation != null) {
					bfDance();
				}
			}
			
			if (curBeat % lilCharDanceEveryNumBeats == 0) {
				if (lilOpp != null && lilOpp.animation != null) {
					oppDance();
				}
			}
		}
		...
		function resetBfBuddy() 
		{
			lilBf.animation.play("idle");
			lilBf.color = FlxColor.WHITE;
		}
		function resetOppBuddy() 
		{
			lilOpp.animation.play("idle");
	...
	function loadSection(?sec:Null<Int> = null)
	{
		...
		if(curSec > 0)
		{
			...
			prevGridBg.rows =Std.int(4 * PlayState.SONG.notes[curSec-1].sectionBeats * curZoom);
		...
		if(curSec < PlayState.SONG.notes.length - 1)
		{
			...
			nextGridBg.rows =Std.int(4 * PlayState.SONG.notes[curSec+1].sectionBeats * curZoom);
		...
		gridBg.rows =Std.int(4 * PlayState.SONG.notes[curSec].sectionBeats * curZoom);
		hei += gridBg.height;
	...
	function addSectionTab()
	{
		...
		mustHitCheckBox = new PsychUICheckBox(objX, objY, 'Must Hit Sec.', 80, function()
		{
			var sec = getCurChartSection();
		...
		objY += 25;
		changeBpmStepper = new PsychUINumericStepper(objX, objY, 1, 0, 1, 999999999, 3); // please you should be able to use custom any bpm - Joseph
	...
	// var diffDropDown:PsychUIDropDownMenu;
	...
	function addSongTab()
	{
		...
		audioOffsetStepper = new PsychUINumericStepper(objX + 180, objY, 1, 0, -9999999999, 9999999999, 0); //new offset lmao!!
		...
		tab_group.add(new FlxText(artistInputText.x, artistInputText.y - 15, 80, 'Artist Name:'));
		tab_group.add(new FlxText(charterInputText.x, charterInputText.y - 15, 80, 'Charter Name:'));
		...
		// diffDropDown = new PsychUIDropDownMenu(objX, objY + 120, [''], function(id:Int, diff:String)
		// {
		// 	// // curDiff = Std.parseInt(PlayState.storyDifficulty);
		// 	// curDiff = PlayState.storyDifficulty;
		// 	// // PlayState.storyDifficulty = curDiff;
		// 	// diff = Std.string(curDiff);
		// 	// trace('selected $diff');
		// // });
		// var setStoryDifficulty = function(diff:String) {
		// 		PlayState.storyDifficulty = Std.parseInt(diff);
		// 		PlayState.SONG = Song.loadFromJson(PlayState.SONG, PlayState.storyDifficulty);
		// 		FlxG.resetState();
		// 	});
		// }
		// diffDropDown.selectedLabel = diffs[PlayState.storyDifficulty];
		...
		// tab_group.add(new FlxText(diffDropDown.x, diffDropDown.y - 15, 80, 'Difficulty:'));
