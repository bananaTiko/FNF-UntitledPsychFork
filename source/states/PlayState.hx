...
import funkin.system.HiddenProcess;
...
class PlayState extends MusicBeatState
{
	...
	public var grpHoldCovers = new FlxTypedGroup<HoldCover>();
	...
	private var hxExt:Array<String> = ['.hx', '.hxs', '.hscript'];
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			case 'stage': new StageWeek1(); 			//Week 1 - Bopeebo, Fresh, Dad Battle
			case 'spooky': new Spooky();				//Week 2 - Spookeez, South, Monster
			case 'philly': new Philly();				//Week 3 - Pico, Blammed, Philly Nice
			case 'limo': new Limo();					//Week 4 - High, Satin Panties, Milf
			...
			case 'phillyBlazin': new PhillyBlazin(new ABotSpeakerFunctions());	//Weekend 1 - Blazin
			case 'backstage': new states.stages.Backstage(); //Week 1 Erect - Bopeebo (Pico Mix), Fresh (Pico Mix), Dad Battle (Pico Mix) /Bopeebo Erect, Fresh Erect, Dad Battle Erect
			case 'spookyMansion': new states.stages.SpookyMansion(new ABotSpeakerFunctions()); //Week 2 Erect - Spookeez (Pico Mix), South (Pico Mix), Monster (Pico Mix) / Spookeez Erect, South Erect, Monster Erect
			case 'phillyErect': new states.stages.PhillyErect(); //Week 3 Erect - Pico (Pico Mix), Blammed (Pico Mix), Philly Nice (Pico Mix) / Pico Erect, Philly Nice Erect, 2Hot Erect
			case 'limoNight': new states.stages.LimoNight(); //Week 4 Erect - High(Pico Mix), Satin Panties (Pico Mix), Milf (Pico Mix) / High Erect, Satin Panties Erect, Milf Erect
			case 'mallErect': new states.stages.MallErect(); //Week 5 Erect - Eggnog (Pico Mix), Cocoa (Pico Mix) / Eggnog Erect, Cocoa Erect
			case 'phillyStreetsErect': new states.stages.PhillyStreetsErect(); 	//Weekend 1 Erect - Darnell (BF Mix), Lit Up (BF Mix), 2Hot (BF Mix)/Darnell Erect, Lit Up Erect, 2Hot Erect
			default: addNene = false;
		}
		if(addNene) new ABotSpeakerFunctions();
		if(isPixelStage) {
	...
	if (curStage.startsWith('mall') || curStage.startsWith('Mall')) {
		...
		#if (LUA_ALLOWED || HSCRIPT_ALLOWED || PYTHON_SCRIPTING)
		...
		for (folder in Mods.directoriesWithFile(Paths.getSharedPath(), 'scripts/'))
			for (file in FileSystem.readDirectory(folder))
			{
				...
				#if HSCRIPT_ALLOWED
				// Check if the file ends with any of the hxExt extensions
				for (ext in hxExt) {
		...
		#if HSCRIPT_ALLOWED startHScriptsNamed('stages/' + curStage + hxExt); #end
		...
		var camLerp = Tools.framerateAdjust(0.04);
		...
		if(ClientPrefs.data.middleScroll) { // make it fell like osu mania but fnf styled
			healthBar.angle = 90;
			healthBar.y = 413;
		}
		...
		updateScore(false);
		if(ClientPrefs.data.middleScroll && ClientPrefs.data.downScroll) { // make it fell like osu mania but fnf styled
			scoreTxt.y = 20;
		...
		#if HSCRIPT_ALLOWED
		for (notetype in noteTypes)
			startHScriptsNamed('custom_notetypes/' + notetype + hxExt);
		for (event in eventsPushed)
			startHScriptsNamed('custom_events/' + event + hxExt);
		#end
		noteTypes = null;
		eventsPushed = null;
...
for (folder in Mods.directoriesWithFile(Paths.getSharedPath(), 'data/$songName/'))
    for (file in FileSystem.readDirectory(folder))
    {
		...
		if(ClientPrefs.data.hitsoundVolume > 0) Paths.sound("hitsounds/hitsound-" + ClientPrefs.data.hitsoundType);
		if(ClientPrefs.data.opphitsoundVolume > 0) Paths.sound("hitsounds/hitsound-" + ClientPrefs.data.oppHitsoundType);
	...
	function startCharacterScripts(name:String)
	{
		...
		#if HSCRIPT_ALLOWED
		var doPush:Bool = false;
		var scriptFile:String = 'characters/' + name + hxExt;
		#if MODS_ALLOWED
	...
	function cacheCountdown()
	{
		...
		var introImagesArray:Array<String> = switch(stageUI) {
			...
			default: ['${stageUI}UIprepare', '${stageUI}UI/ready', '${stageUI}UI/set', '${stageUI}UI/go'];
		}
	...
	// {
	// 	final epics:Int = ratingsData[0].hits;
	// 	final sicks:Int = ratingsData[1].hits;
	// 	final goods:Int = ratingsData[2].hits;
	// 	final bads:Int = ratingsData[3].hits;
	// 	final shits:Int = ratingsData[4].hits;
	
	// 	ratingFC = "Error"; //rating's aren't loading or aren't working
	// 	if(songMisses == 0)
	// 	{
	// 		if (sicks > 0 || epics > 0) ratingFC = 'SFC'
	// 		else if (shits > 0) ratingFC = 'FC';
	...
	function startSong():Void
	{
		...
		if(autoUpdateRPC) DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter(), true, songLength - Conductor.songPosition);
		#end
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		for (i in 0...4)
		{
			...
			if (player < 1)
			{
				...
				else if(ClientPrefs.data.middleScroll) targetAlpha = 0;
			}
	...
	function resetRPC(?showTime:Bool = true)
	{
		#if DISCORD_ALLOWED
...
// Assuming camNotes and camRating are already initialized as separate cameras:
function syncCameras():Void {
    ...
    camRating.x = camHUD.x;
    camRating.y = camHUD.y;
	...
	override public function update(elapsed:Float)
	{
		...
		syncCameras(); // Synchronize cameras every frame
		...
		if (startingSong)
		{
		...
		else if (!paused && updateTime)
			{
				...
				var secondsLeft:Int = Math.floor(songCalc / 1000 / playbackRate);
				if (secondsLeft < 0) secondsLeft = 0;
			
				if (ClientPrefs.data.timeBarType != 'Song Name') {
					timeTxt.text = FlxStringUtil.formatTime(secondsTotal, false);
				} else if (ClientPrefs.data.timeBarType != 'Time Elapsed' && ClientPrefs.data.timeBarType != 'Time Left' && ClientPrefs.data.timeBarType == 'Modern Time') {
					timeTxt.text = FlxStringUtil.formatTime(secondsLeft, false) + " / " + FlxStringUtil.formatTime(secondsTotal, false);
				} else { // this is what was messed up, hopefully this fixes it.
					timeTxt.text = FlxStringUtil.formatTime(secondsTotal, false);
				}
			}
		...
		if (camZooming)
		{
			...
			camHUD.zoom = FlxMath.lerp(1, camHUD.zoom, Math.exp(-elapsed * 3.125 * camZoomingDecay));
		}
				...
				if(notes.length > 0)
				{
					if(startedCountdown)
					{
						...
						notes.forEachAlive(function(daNote:Note)
						{
							...
							else if (daNote.wasGoodHit && !daNote.hitByOpponent && !daNote.ignoreNote  && (daNote.isSustainNote || daNote.strumTime <= Conductor.songPosition))
	...
	{
		var mult:Float = FlxMath.lerp(1, ClientPrefs.data.middleScroll ? iconP1.scale.y : iconP1.scale.x, Math.exp(-elapsed * 9 * playbackRate));
		iconP1.scale.set(mult, mult);
		iconP1.updateHitbox();

		var mult:Float = FlxMath.lerp(1, ClientPrefs.data.middleScroll ? iconP1.scale.y : iconP2.scale.x, Math.exp(-elapsed * 9 * playbackRate));
	...
	public dynamic function updateIconsPosition()
	{
		...
		if(ClientPrefs.data.middleScroll){
			iconP1.x = iconP2.x = healthBar.x + 230;
			iconP1.y = (healthBar.width * (FlxMath.remapToRange(healthBar.percent, 0, 110, 110, 0) * 0.01) - iconOffset);
			iconP2.y = (healthBar.width * (FlxMath.remapToRange(healthBar.percent, 0, 115, 115, 0) * 0.01)) - (iconP2.width - iconOffset);
		}
		else {
		...
		switch(eventName) {
			...
			case 'Play Animation':
		 	...
		 	case 'Windows Notification':
				if (value1 == "") value1 = "Untitled Psych Fork";
				if (value2 == "") value2 = "forgot to change values lmao";
			...
			case 'Change Scroll Speed':
				if (songSpeedType != "constant")
				{
					...
					if(flValue2 <= 0)
						...
						songSpeedTween = FlxTween.tween(this, {songSpeed: newValue}, flValue2, {ease: FlxEase.linear, onComplete:
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		totalNotesHit += (daRating.name == 'epic' ? 1 : ratingsData[0].hitWindow / (noteDiff/playbackRate));
		...
		comboSpr.acceleration.y = FlxG.random.int(200, 300) * playbackRate;
	...
	function noteMissCommon(direction:Int, note:Note = null)
	{
		...
		if(lastCombo > 5) {
			...
			broke.y -= 60;
			broke.acceleration.y = FlxG.random.int(200, 300) * playbackRate;
			broke.velocity.y -= FlxG.random.int(140, 160);
			broke.velocity.x = FlxG.random.float(-5, 5);
			...
			{
				broke.setGraphicSize(Std.int(broke.width * daPixelZoom * 0.85));
			}
		}
	}
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if (note.opphitsoundVolume > 0 && !note.opphitsoundDisabled  && !note.isSustainNote)
			FlxG.sound.play(Paths.sound(note.opphitsound), note.opphitsoundVolume);
...
public function updateRatingFC() {
    if (ratingsData == null || ratingsData.length < 5) {
        trace("ratingsData is not properly initialized.");
        ratingFC = '';
        return;
    }
    ...
    if (songMisses == 0) {
        if (bads > 0 || shits > 0) ratingFC = 'FC'; // Full Combo (some misses but still counted as FC)
        else if (goods > 0) ratingFC = 'GFC'; // Great Full Combo (some good hits, no misses)
        else if (sicks > 0) ratingFC = 'SFC'; // Sick Full Combo (some sick hits)
        else if (epics > 0) ratingFC = 'PFC'; // Perfect Full Combo (only perfect hits)
    } else {
        // With misses
        if (songMisses <= 2) {
            ratingFC = 'NFC'; // Near Full Combo (barely missed Full Combo)
        } else if (songMisses <= 10) {
            ratingFC = 'AFC'; // Average Full Combo (more mistakes but still completed)
        } else if (songMisses <= 12) {
            ratingFC = 'DCB'; // Decent Combo Break (lost combo in a few places but not too many)
        } else if (songMisses <= 15) {
            ratingFC = 'LCB'; // Low Combo Break (missed several notes but not overwhelming)
        } else if (songMisses <= 19) {
            ratingFC = 'SDCB'; // Single Digit Combo Break (missed more notes, but still low misses)
        } else {
            ratingFC = 'Clear'; // Clear (completed the song with several misses)
        }

        // For extremely poor performance (high number of misses)
        if (songMisses > 19) {
            ratingFC = 'Pass'; // Completed with many misses
        }
    }
}
	#if ACHIEVEMENTS_ALLOWED
