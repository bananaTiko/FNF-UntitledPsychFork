...
class MainMenuState extends MusicBeatState
{
	...
	public static var funkinVersion:String = '0.5.3'; // Version of funkin' we are emulatioing 
	...
	override function create()
	{
		...
		//UPF version shit
		var upfVer:FlxText = new FlxText(12, FlxG.height - 64, 0, "Untitled Psych Fork v" + untitledPsychForkVersion, 12);
		...
		if (leDate.getDay() == 1 && leDate.getHours() >= 18)
			Achievements.unlock('monday_night_play');
		...
		if (leDate.getDay() == 6 && leDate.getHours() >= 18)
			Achievements.unlock('saturday_night_play');
		if (leDate.getMonth() == 9)
			Achievements.unlock('spooky_month');
		if (leDate.getMonth() == 11 || leDate.getMonth() == 10)
			Achievements.unlock('spooky_before_christmas');
		...
		{
			case 'VS Impostor' | 'VS Nonsense V2' | 'Neo' | 'B Sides' | 'Shaggy'| 'IC' | 'JSR'  |'(OST Version)': 
			Conductor.bpm = 102;
			case 'B Sides Redux':
			Conductor.bpm = 88;
			case 'Mario Madness':
	...
	var selectedSomethin:Bool = false;
		...
		function tipTextStartScrolling()
			...
			function changeTipText() {
				...
				tipText.alpha = 1;
				isTweening = true;
				selectedText = textArray[FlxG.random.int(0, Std.int(textArray.length - 1))].replace('--', '\n');
	...
	override function beatHit()
	{
		...
		FlxG.camera.zoom += 0.015; //zoom yeah
