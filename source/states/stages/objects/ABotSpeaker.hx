...
class ABotSpeaker extends FlxSpriteGroup
{
	...
	public function new(x:Float = 0, y:Float = 0,addDarkSprite:Bool = false)
	{
		super(x, y);
		...
		speaker = abotLol(addDarkSprite);
		if(addDarkSprite) {
			darkSpeaker = abotLol(false);
			darkSpeaker.alpha = 0;
		}
	}

	function abotLol(addDarkSprite:Bool) {
		var temp = new FlxAnimate(-65, -10);
		Paths.loadAnimateAtlas(temp, '${addDarkSprite? "abot/dark" : "abot"}/abotSystem');
